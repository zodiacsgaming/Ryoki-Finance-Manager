...
import { Plus, Pencil, Trash2, Shield, Target, CheckCircle2, PlusCircle, ArrowLeftRight } from 'lucide-react'
...
export default function EmergencyFundPage() {
  ...
  const [transferringItem, setTransferringItem] = useState<EmergencyFund | null>(null)
  ...
  const openTransfer = (fund: EmergencyFund) => {
    setTransferringItem(fund)
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    if (isNaN(amount) || amount <= 0) { toast.error('Enter a valid amount'); return }
    if (amount > transferringItem.current_amount) { toast.error('Amount exceeds available fund balance'); return }

    setTransferring(true)
    try {
      const newAmount = transferringItem.current_amount - amount
      const [fundRes, cashRes] = await Promise.all([
        fetch(`/api/data/emergency-funds/${transferringItem.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ current_amount: newAmount }),
        }),
        fetch('/api/data/cash-on-hand', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            description: `Transfer from emergency fund: ${transferringItem.name}`,
            amount,
            type: 'in',
                ...
                <div key={fund.id} className="card p-5 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex gap-1 shrink-0">
                      ...
                      <button onClick={() => openTransfer(fund)} title="Transfer to Cash on Hand" className="p-1.5 rounded-lg hover:bg-orange-50 dark:hover:bg-orange-900/30 text-gray-400 hover:text-orange-600 transition-colors"><ArrowLeftRight className="w-3.5 h-3.5" /></button>
        ...
        {transferringItem && (
          <form onSubmit={handleTransfer} className="space-y-4">
            <div className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-4">
              <p className="text-xs text-gray-400 mb-1">From Emergency Fund</p>
              <p className="font-semibold text-gray-900 dark:text-white">{transferringItem.name}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Available: <span className="font-medium text-purple-600 dark:text-purple-400">{formatCurrency(transferringItem.current_amount)}</span>
              </p>
            </div>
            <div>
              ...
              {transferForm.amount && !isNaN(parseFloat(transferForm.amount)) && parseFloat(transferForm.amount) > 0 && (
                <p className="text-xs text-orange-600 dark:text-orange-400 mt-1">
                  Fund remaining: {formatCurrency(transferringItem.current_amount - parseFloat(transferForm.amount))}
                </p>
              )}
            </div>
            <div>
